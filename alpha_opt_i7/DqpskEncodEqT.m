function [ EoutI,EoutQ ] = DqpskEncodEqT( TxData,PreBits )
...
%inputs and if they are accordingly with the expected.
    if nargin<2                                                            %If the input given was just the Txdata set the start point
        PreBits = zeros(2,size(TxData,2));
    end
    EoutI = zeros(size(TxData,1)/2,size(TxData,2));                                                            %Initializing the I output.
    EoutQ = zeros(size(TxData,1)/2,size(TxData,2));                                                            %Initializing the Q output.
    ...
    for kk=1:length(ResA)                                                  %Actualy performing the equation for each pair of bits
        ...
        pk = ((~a)&(~b)&(~pk_1))|((~a)&(b)&(qk_1))|((a)&(~b)&(~qk_1))|((a)&(b)&(pk_1));
        %Calculating the current Q bit
        qk = ((~a)&(~b)&(~qk_1))|((~a)&(b)&(~pk_1))|((a)&(~b)&(pk_1))|((a)&(b)&(qk_1));
