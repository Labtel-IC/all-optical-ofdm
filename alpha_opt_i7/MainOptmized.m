...
PrintinEye    = OFF;                                                       %Printing Eye Diagram of modulations
PlotingFD     = OFF;
...
CalcS         = ON;                                                        %Set 1 to enable the first eye openeing stimative
...
TimeSys       = OFF;                                                        %Select which time period will be accounted to the noise, set 1 to use T or set 0 to use t(2*NumAmosCP+NPPB).
...
CurrentMedium = OFF;                                                        %Sellection of channel 1 for Fiber other of B2B
UseOfdmElect  = OFF;
ChanAwgn      = OFF;
...
NumbOfTest    = 3000/CurTesPas;                                              %The number of repetition that one test will be done
PAM4Type      = [0 0];                                                     %Sellection the type of 4PAM that will be tested
MinTesNumb    = 200/CurTesPas;
MinSavTim     = 15;
...
DecLevDef3 = 0.8296;                                                       %Uper levels of PAM4 - between level 3 and level 4
DecLevDef2 = 0.4903;                                                       %Uper levels of PAM4 - between level 2 and level 3
DecLevDef1 = 0.1663;                                                       %Uper levels of PAM4 - between level 1 and level 2
...
% work.
    if ~mod(RefCarr,NumCarr)                                               %Luckly if the first carrier was a multiple of the number of used carriers
        ...
        SaveBerEbn0SuFix    = '_BerVsEbnONoOverSamp_2';
        SaveBerEbn0FigSuFix = '_BerVsEbnONoOverSamp_fig_2.fig';
...
switch Modulation
    ...
    case 'OFDM'
        ...
        %modulation.
        UsingHermitian = 1;                                                %Turn On or Off the usage of Hermitian Symetri
        if ~UseOfdmElect                                                   %Whenever a non-electrical OFDM is used the Hermitian Symetri in needed on this script
            UsingHermitian = 1;
        end
...
VetSnrIni     = 5;                                                        %Start of the main Loop
VetSnrPass    = 1;                                                       %Main Loop pass
VerSnrEnd     = 22;                                                        %Stop of the main Loop
ThisPlotCont  = 1;                                                      
...
for VetSnr=VetSnrIni:VetSnrPass:VerSnrEnd
    ...
    SavingAT     = [pwd '\..\results\' 'Pam4EleBerVsSnrFulSys_' num2str(VetSnr) '_3'];
%     SavingAT     = [pwd 'test0'];
    ...
    switch Modulation
        ...
        case 'DPSK' %Tested to fiberlength of 80 -> worked
            if TimeSys==1
                ...
                SNR = CarSNR + 10*log10(1) - 10*log10(1*t(2*NumAmosCP+NPPB,1)*BWD2) - 10*log10(2/3);
            end
        ...
        case 'DQPSK'%Tested to fiberlength of 40 -> work in progress
            if TimeSys==1
                ...
                SNR = CarSNR + 10*log10(2) - 10*log10(t(2*NumAmosCP+NPPB,1)*BWD2) - 10*log10(sqrt(2)*2/3);
            end
            TxSymbAmos = zeros(NumbOfTest,NumCarr,Nb - NumBitDesc);
        case '4PAM'%Tested to fiberlength of 1 -> work in progress
            if TimeSys==1
                ...
                SNR = CarSNR + 10*log10(2) - 10*log10(1*t(2*NumAmosCP+NPPB,1)*BWD2) - 10*log10(1/4/sqrt(2));
            end
            TxSymbAmos = zeros(NumbOfTest,NumCarr,Nb - NumBitDesc);
        otherwise %Tested to fiberlength of 80 -> worked
            if TimeSys==1
                SNR = CarSNR + 10*log10(1) - 10*log10(1*T*BWD2)- 10*log10(0.25);
            else
                SNR = CarSNR + 10*log10(1) - 10*log10(1*t(2*NumAmosCP+NPPB,1)*BWD2) - 10*log10(1/4);
            end
            TxSymbAmos = zeros(NumbOfTest,NumCarr,Nb - NumBitDesc);
    end
                        ...
                        for kk=InitCarrDo:CarrPass:NumCarr%Generating different data for each carrier
                            if CarrUsedDo(kk)% Frist it is chosen to transmite a random information...
                                ...
                                if Which4PAM==1%Selecting if the PAM will be done on Electrical or Optical domain
                                    %  Good for others kms
                                    [Phi1,Phi2] = Maping4PamIqT(TxData,Vmin,Vmax,ModSchem,FiberLength,SetCpSampZer,ReceptorNoise);%Generating the eletrical signal for the optical PAM4
                                else
                                ...
                                if PlotingThis
                                    Olho(TxSig1(1:end-(StuffSampels)),t(2*NumAmosCP+NPPB),(2*NumAmosCP+NPPB),1,1);
                                end
                                %Because of reasons, the real world does not have components capable
                                %instantly change the voltage level. Thus, to emulate this behaviour the
                                %eletrical PAM signal pass through a gaussian filter that will remove the
            ...
            if SendingDowStr==1
                ...
                if FFTSplit==1
                    ...
                    if SendingUp==1%&&(NumCarr>2)
                        for jj=1:size(EoutRec,2)
                            EoutAux1G    = SelectEachCarrier(EoutRec(:,jj),NumCarr,f(:,1),fin,1.0*fc,11,fc);
                            EoutAux1(:,jj,:) = EoutAux1G.';
                            clear EoutAux1G;
                        end
                    else
                        EoutAux1(:,1:size(EoutRec,2),1)    = EoutRec;
                        EoutAux1(:,1:size(EoutRec,2),2)    = 0;
                    end
                end
...
%                                 if PlotingThis
%                                     Olho(Ix(1:end-(StuffSampels)),t(2*NumAmosCP+NPPB),(2*NumAmosCP+NPPB),1,1);
%                                 end
%                                 if ThisCarr==1
%                                     sn=((1/2)*(((1/2)^MaxNumStag)-1))/((1/2)-1);
%                                     AuxSyncCorr = IfftOrSum*round((sn/(2/2^IfftOrSum))*T/Ta);
%                                     Ix1 = [Ix(AuxSyncCorr+1:end,:);Ix(1:AuxSyncCorr,:)];
%                                     IxAux = Ix1(1:end - StuffSampels,:);
%                                     Olho((IxAux(:)),t(2*NumAmosCP+NPPB),(2*NumAmosCP+NPPB),1,4);
%                                     EyeToPlot(CurrentTest,1:length(IxAux(:))) = IxAux(:);
%                                     NPPBR = 2*NumAmosCP+NPPB;
%                                     TR = t(2*NumAmosCP+NPPB);
%                                     save(['savingforplotingeye' num2str(VetSnr)],'EyeToPlot','VetSnr','IfftOrSum','UsedModula','TR','NPPBR');
%                                 end
								if UsingMex==1
        ...
        if (UseOfdmElect)&&(UsedModula==5)&&(NumCarr==2)
            ...
            if SendingUp
                ...
                if FFTSplit==1
                    ...
                    VetThisCarr = ObsCarrPos;
                    for jj=1:size(EoutRec,2)
                        EoutAux1G    = SelectEachCarrier(EoutRec(:,jj),NumCarr,f(:,1),fin,1.0*fc,5,fc);
                        EoutAux1(:,jj,:) = EoutAux1G.';
                        clear EoutAux1G;
                    end
...
%                                 close all;
        ...
        if CurrentModula == 2
            ...
            BerNotZer = find(Ber4PAM(:,2));
            if isempty(BerNotZer)
                BerNotZer = 1:size(Ber4PAM,2);
            end
            b = Ber4PAM(1:max([max(BerNotZer) CurrentTest]),:);
            
            ElapsedTime = toc - PastTime;
            PastTime = toc;
            if PastTime/60 > MinSavTim
                Ber4PAM = Ber4PAM(1:max([max(BerNotZer) CurrentTest]),:);
                if CurrentModula == UpStModula
                    ...
                    save(SavingAT,'NumbOfTest',...
                        ...
                        'EvmPerJ','EvmRmsJ','TxSymbAmos','RxSymbAmos','CarSNR','SNR');
                end
                PastTime = 0;
                tic;
            end
        elseif CurrentModula == 1
            ...
            BerNotZer = find(BerOOK(:,2));
            if isempty(BerNotZer)
                BerNotZer = 1:size(BerOOK,2);
            end
            b = BerOOK(1:max([max(BerNotZer) CurrentTest]),:);
            
            ElapsedTime = toc - PastTime;
            PastTime = toc;
            if PastTime/60 > MinSavTim
                BerOOK = BerOOK(1:max([max(BerNotZer) CurrentTest]),:);
                save(SavingAT,'NumbOfTest',...
        ...
        elseif CurrentModula == 3
            ...
            BerNotZer = find(BerDQPSK(:,2));
            if isempty(BerNotZer)
                BerNotZer = 1:size(BerDQPSK,2);
            end
            b = BerDQPSK(1:max([max(BerNotZer) CurrentTest]),:);
            
            ElapsedTime = toc - PastTime;
            PastTime = toc;
            if PastTime/60 > MinSavTim
                BerDQPSK = BerDQPSK(1:max([max(BerNotZer) CurrentTest]),:);
        ...
        elseif CurrentModula == 4
            ...
            BerNotZer = find(BerDPSK(:,2));
            if isempty(BerNotZer)
                BerNotZer = 1:size(BerDPSK,2);
            end
            b = BerDPSK(1:max([max(BerNotZer) CurrentTest]),:);
            
            ElapsedTime = toc - PastTime;
            PastTime = toc;
            if PastTime/60 > MinSavTim
                BerDPSK = BerDPSK(1:max([max(BerNotZer) CurrentTest]),:);
                save(SavingAT,'NumbOfTest','BerDPSK','ValsLev',...
                    ...
                    'EvmRmsJ','TxSymbAmos','RxSymbAmos','CarSNR','SNR');
                PastTime = 0;
                tic;
            end
            ...
            if (UseOfdmElect)&&(UsedModula==5)&&(NumCarr==2)
                ...
                if PastTime/60 > MinSavTim
                    save(SavingAT,'NumbOfTest','BerOFDM','OSNRPC','OfcName','DmtKvep',...
                        ...
                        'SelecGaus','NuDmSuDi','OfdMod','SelModTp','DmtPas','DmtMinM','CarSNR','BerToPlotOfdm','SNR');
                    PastTime = 0;
                    tic;
                end
            end
        ...
        display(sprintf(['Elapsed Time(s) = %f | Current Test = %d '...
            ...
            0.8*Nb)),(5/min(mean(b(:,berpos1),1)))));
        a=a+1;
        if CurrentTest > MinTesNumb
            if (((CurrentTest)*ceil(0.8*Nb))>(5/min(mean(b(:,berpos1),1))))
                break;
            end
        end
    end
    if CurrentModula == 2
        BerNotZer = find(Ber4PAM(:,2));
        if isempty(BerNotZer)||(CurrentTest>=NumbOfTest)
            BerNotZer = 1:size(Ber4PAM,2);
        end
        Ber4PAM = Ber4PAM(1:max([max(BerNotZer) CurrentTest]),:);
        if CurrentModula == UpStModula
    ...
    elseif CurrentModula == 5
        if (UseOfdmElect)&&(UsedModula==5)&&(NumCarr==2)
            save(SavingAT,'SelecGaus','OfdMod','SelModTp',...
                ...
                'EvmRms','EvmDBJ','EvmPerJ','EvmRmsJ','SaveRxEq','SaveRxNotEq','SNR');
        else
            save(SavingAT,'NumbOfTest','BerOFDM','OSNRPC','OfcName','DmtKvep',...
                'RefCarr', 'CarrUsed','SelecSetUP','FFTSplit','CurrentMedium','OfdMod',...
                'UsedModula','NumCarr','PAM4Type','UpStModula','IfftOrSum','NumCarr',...
