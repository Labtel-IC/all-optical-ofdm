...
if FFTSplit
    ...
    EoutAux1    = SelectEachCarrier(EoutRec,NumCarr,f,fin,1.2*fc,5,fc);
end
    ...
    case 'DPSK'
        ...
        for ThisCarr=InitCarr:CarrPass:NumCarr                                    %For each carrier the same process of reception will be used.
            if CarrUsedUp(ThisCarr)
                ...
                SyncAux(SyncAux>=mean(Esync)) = 1;                           %Adding a flag to the first sample of the received mean value
                SyncAux(SyncAux<mean(Esync))  = -1;                          %All the others samples at set to the lowest level
        ...
        for ThisCarr=InitCarr:CarrPass:NumCarr                                           %For each carrier the same process of reception will be used.
            if CarrUsedUp(ThisCarr)
                ...
                if ReceptorNoise                                               %Verify whether the noise is to be added or not
                    ...
                    SigPowerI2 = SigPowerI-30;%20*log10(SigPowerI);
                    SigPowerQ2 = SigPowerQ-30;%20*log10(SigPowerQ);
...
%                     OccuCountQ)))));
                ...
                %##############################################################
                %##############################################################
                %             PrintInfo(PlotingThis*24,EoutQ,T,NPPB);
                %             hold all;
                %             plot(t(NPPB/2),EyeOpenLowQ+EyeOpenQ/2,'bd');plot(t(NPPB/2),EyeOpenLowQ,'bo');plot(t(NPPB/2),EyeOpenHighQ,'bx');
                %             plot([t(SymLocQ(1)) t(SymLocQ(1))],[-1 1]);
    ...
    case '4PAM'
        ...
        for ThisCarr=InitCarr:CarrPass:NumCarr                                           %For each carrier the same process of reception will be used.
            if CarrUsedUp(ThisCarr)
                ...
                %signal must be about the noise for an acceptable recovering.
                if ReceptorNoise                                               %Verify whether the noise is to be added or not
                    [~,SigPower] = MeasPower(Ix-min(Ix));                              %it own received signal or
                    SigPower2 = SigPower-30;%10*log10(SigPower);
...
%                             drawnow;
                ...
                Ber4PAM(CurrentTest,ThisCarr) = BitErr/(Nb4Pam-(2*...
    ...
    otherwise
        ...
        for ThisCarr=InitCarr:CarrPass:NumCarr                                           %For each carrier the same process of reception will be used.
            if CarrUsedUp(ThisCarr)
                ...
                %                             PrintInfo(PlotingThis*45,t(IniSyncPos:SyncPos),Ix(IniSyncPos:...
                %                                                     SyncPos),SyncSymb(IniSyncPos:SyncPos));
%                                             PrintInfo(PlotingThis*46,t(1:length(Ix)),Ix);
                ...
                IxRec    = Ix(IxPosAux);
                if CurrentModula==2
                    [EvmRmsUp(CurrentTest,ThisCarr),EvmDBUp(CurrentTest,ThisCarr),...
                        EvmPerUp(CurrentTest,ThisCarr)] = EvmCalc( EvmMatRef(ThisCarr,...
                        1:length(IxPosAux)),IxRec );
                else
                    [EvmRms(CurrentTest,ThisCarr),EvmDB(CurrentTest,ThisCarr),...
                        EvmPer(CurrentTest,ThisCarr)] = EvmCalc( EvmMatRef(ThisCarr,...
                        1:length(IxPosAux)),IxRec );
...
%##########################################################################
                ...
                %three level of decissions that are exaclty the center of the
                %eye diagram.
                Interval = linspace(-1*Perce*(abs(abs(max(IxAux))-abs(min(IxAux)))/2),(Perce*abs(abs(max(IxAux))-abs(min(IxAux)))/2),n);                     %Building the histogram boxes
                
                %Therefore, the MATLAB hist function returns the number of
                %occurrence of each interval.
                EyeMax = hist(Ix,Interval);
                EyeMaxaux = [0 EyeMax 0];                                      %Zeros are added at the EyeMax to auxiliate the finding peaks process
                [~,EyeLoc] = findpeaks(EyeMaxaux,'SortStr','descend','NPeaks',2,'MinPeakHeight',mean(EyeMax));%The peaks on the Eye profile will be the levels at the Eyes limit
                if length(EyeLoc)<2
                    [~,EyeLoc] = findpeaks(EyeMaxaux,'SortStr','descend','NPeaks',2,'MinPeakHeight',mean(EyeMax)/8);%The peaks on the Eye profile will be the levels at the Eyes limit
                end
...
%                     ))));%The peak on the Eye profile will be the Symmetry level
                ...
                if CurrentModula==2
                    ...
                    if BitErr2<=BitErr
                        Ber4PAM(CurrentTest,ThisCarr) = BitErr2/((Nb-NumBitDesc)-...
                        2*SyncPeriod);%Calculating the ration of wrong bits and the total number of bits transmited
                        AberLevUp(CurrentTest,ThisCarr)= Inter2(SeqFin2(LocMax2))-Inter2(round(SeqFin2(LocMax2)-MaxVal2));
                        ValsLevUp(CurrentTest,ThisCarr)= LevDec2;
                    else
                        Ber4PAM(CurrentTest,ThisCarr) = BitErr/((Nb-NumBitDesc)-...
                        2*SyncPeriod);%Calculating the ration of wrong bits and the total number of bits transmited
                    end
                else
                    BitErr = sum(xor(TxDataMat(ThisCarr,1+SyncPeriod:end-...
