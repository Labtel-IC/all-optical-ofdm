function  [Eout,H] = Mach_Zehnder_Modulator_Ezequiel(t,Ein,U,MZ_Input_File)
%c
...
if (nargin <= 3),
  ...
  load([MZ_Input_File '.m']);
   ...
   [L,U0,~,~,~,~,nopt,nel,~,phi_0,C,alfa0]=Import_Data_MZM (MZ_Input_File);%Import the data from files 
end
U_pi1 = 4.7;
U_pi2 = 4.7;
if isstruct(U),
...
if (electrodes == 1),
  ...
  if C~=0
      Eout  = Ein.*cos((pi/2).*(U1t - U0 + 2.78)/U_pi).*exp1;
  else
      Eout  = Ein.*cos((pi/2).*(U1t - U0 + 2.78)/U_pi);
  end
else
  U1f   = fft(U1t);
  U1t   = ifft(U1f.*H);
  ...
  U2t   = ifft(U2f.*H);
  if C ~= 0
      Eout = Ein.*cos((pi/2).*(U1t - U2t - U0 + 2.78)/U_pi).*exp(-1j*(pi/2).*((U1t + U2t)/U_pi));
  else
      Eout = Ein.*cos((pi/2).*(U1t - U2t - U0 + 2.78)/U_pi);
  end
end
