...
if ~(isempty(mais10pic))                                                   %Check is there are carriers to be evaluate, if not the evaluation is done
    if Ploting
        figure;
        hold all;
        loc_aux = pontos(locmais10pic);
        for ll=1:length(mais10pic)
    ...
    save(LocalSaved,'melhorespontos','representa','BestPlanicity');        %Saving the results
end
a=1;