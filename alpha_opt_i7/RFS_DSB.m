...
for kk=1:Vbias_steps
    for jj=1:Vbias_steps
        
        EleSig2           = Vbias(kk)*sin(2*pi*f_RF*t + phase_is);
        EleSig1           = Vbias(jj)*sin(2*pi*f_RF*t);
...
%    end
    ...
    % Mach Zhender Modulator
        [Eout,H]=Mach_Zehnder_Modulator_simplificado(t,CW,EleSig,MZ_Input_Sdata);
        ponto_max_db = max(20*log10(abs(fftshift(fft(Eout)./length(Eout)))));
        limiar_db = ponto_max_db - 5;
        [peaks(S,:),locs(S,:)] = findpeaks(20*log10(abs(fftshift(fft(Eout)./length(Eout)))),'MinPeakHeight',limiar_db);
        S = S + 1;
    end
end
