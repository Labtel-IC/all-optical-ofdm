...
EleSig1     = Vbias*sin(2*pi*fc*t);               %The first RF signal at the Uper arm.
EleSig2     = Vbias*sin(2*pi*fc*t);    %The second RF singal at the Lower arm.
%%                    Main Loop
for N=1:N_volta
   ...
   if InfinitLoop
    ...
    [Eout(S,:)] = IqModOCS (E1(S,:),EleSig1,EleSig2,U_pi2,Vbias);%,t);
...
%        end
       figure(1);
%        subplot(3,1,2),
       plot(f,db(abs((fftshift(fft(Eout))))/length(...
                                                              Eout)));
