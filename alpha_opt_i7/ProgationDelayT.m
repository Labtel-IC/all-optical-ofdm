function [Delay,PowRef] = ProgationDelayT(CarrRef,NumOfCarrier,temp,lambda,To,L,regime,f...
                                      ,Ploting,FiberInpuData,FiberFunction)
    %%
    if nargin < 8
    ...
    NPPB = round(To/(temp(2,1)-temp(1,1)));
    ...
    VetDif = length(TxPul)-size(temp,1);

    if VetDif > 0 
        TxPul = [TxPul;zeros(VetDif,1)];
    elseif VetDif < 0
        TxPul = TxPul(1:end-VetDif,1);
    end
    Delay  = zeros(NumOfCarrier,1);
    PowRef = zeros(NumOfCarrier,1);
    for kk=0:NumOfCarrier-1
        TxPul = ifft(fft(TxPul).*Filt);
        PiloPuls = TxPul.*exp(1j*2*pi*((CarrRef+kk)/To).*temp);
        %%
        ...
        Delay(1+kk)  = PosToSync - SyncedPos;
        PowRef(1+kk) = sum(abs(Ix).^2)./length(Ix);
%         a=1;
    end
end