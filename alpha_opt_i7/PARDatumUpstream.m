...
PARDatumUpstreamInputData;
clear TxData TxDataMat TxDataMat1 TxDataMat2
...
switch Modulation
    case 'DPSK'
        ...
        for kk=1:NumCarr%Generating different data for each carrier
...
switch Modulation
    case 'DPSK'
        ...
        parfor kk=1:NumCarr
            if ~mod(kk,2)
                EoutTx = EoutAux1(VetThisCarr==kk,:);
                ...
                %to zero.
                [EoutModAux,~]=MZM_Simp_Parfor(t,EoutTx,U1t,U2t,...
...
%                 end
    ...
    case 'DQPSK'
        ...
        parfor kk=1:NumCarr
            if ~mod(kk,2)
                EoutTx = EoutAux1(VetThisCarr==kk,:); 
                [EoutModAux] = IqMod(EoutTx,DatGai.*TxSigMat1(kk,:),DatGai.*TxSigMat2(kk,:),Vpi,V0);
                EoutMod = EoutMod + EoutModAux;                            %Adding the current Modulated output to the final OutPut
            end
        end
    case '4PAM'
        ...
        parfor kk=1:NumCarr
            if ~mod(kk,2)
                ...
                if Which4PAM
                    if ModSchem
                        [EoutModAux] = IqMod4Pam (EoutTx,U1t,U2t,U_pi2,...
                                                                    Vbias);
                    else
                        [EoutModAux] = MZM_Simp_Parfor(t,EoutTx,U1t,U2t,...
                                                            MZ_Input_File);
                    ...
                    [EoutModAux] = MZM_Simp_Parfor(t,EoutTx,U1t,U1t,...
                                                            MZ_Input_File);
                end
                EoutMod = EoutMod + EoutModAux;                            %Adding the current Modulated output to the final OutPut
                ...
                EvmMatRef(kk,:) = [IxAux(PosAuxEout) 1:NumBitDesc];        %Taking just the middle samples as references
%                 if kk == 1
    ...
    otherwise
        ...
        parfor kk=1:NumCarr
            if ~mod(kk,2)
                ...
                %configuration. It is necessary to reduce the Chirp noise 
                %to zero.
                [EoutModAux,~]=MZM_Simp_Parfor(t,EoutTx,U1t,U2t,MZ_Input_File);%Modulating individual carriers
                EoutMod = EoutMod + EoutModAux;                            %Adding the current Modulated output to the final OutPut
                %% Taking the sampling the EVM meassurement
                PosAuxEout      = (2*NumAmosCP+NPPB)/2:(2*NumAmosCP+NPPB...
                                       ):(length(EoutModAux)-StuffSampels);%Varriable respossible to take just the samples at the middle of the symbol
