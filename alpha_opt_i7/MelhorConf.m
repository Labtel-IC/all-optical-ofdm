...
clear ; clc; close all;
ver_melhor_conf = 1;
load([pwd '\save_files\TwoArmNoLoopFc1G_' '09-Oct-2017' '.mat']);          %Load the result of the past simulation for evaluation
%%
...
%              Gain = Gain_vet(3);
            for jj=1:Gain_steps
                Gain = Gain_vet(jj);
                 for mm=1:Varm1_steps%For each voltage apply at the second arm
                    for kk=1:Varm2_steps%and at the first arm
                        if find(ismember(melhorespontos,S))                    %Check if it is a optimum point
                            ...
                            melhor_va1 = [melhor_va1 Varm1(mm)];
                            melhor_va2 = [melhor_va2 Varm2(kk)];
                ...
                display((S-1)*100/(phase_steps*Vbias_steps*Vbias_steps*...
