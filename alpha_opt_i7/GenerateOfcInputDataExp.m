...
Filter_Center_Freq = 20*fc; %center frequency of the gausian filter
FilterBand         = 40*fc;  %pass band of the gausian filter
N_volta            = 196;     %The number of times that the CW will 
                              %recirculate
EbN0               = 40;      %power of the bit to the noise flor
NF                 = 4.5;     %Noise Figure
PWsat              = 22;      %Power limit
...
Eo_CW              = 0.1;                 %Amplitude of the CW signal
CW                 = ones(1,length(t));
% CW                 = cos(2*pi*fc.*t);
% CW                 = exp(1j*2*pi*fc*t);
...
Gain_vet      = 10*log10(6);%2.177;%linspace(1,2,Gain_steps);                           %Variation of Gain inside the optical ring for CW = ones
% Gain_vet      = 10*log10(4.225);%2.177;%linspace(1,2,Gain_steps);                           %Variation of Gain inside the optical ring
