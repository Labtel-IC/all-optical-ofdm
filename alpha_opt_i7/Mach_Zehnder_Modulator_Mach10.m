function  [Eout,H] = Mach_Zehnder_Modulator_Mach10(t,Ein,U,MZ_Input_File)
%c
...
if (electrodes == 1),
  ...
  if C~=0
      Eout  = Ein.*cos((pi/2).*(U1t - U0 + 4.1)/U_pi).*exp1;
  else
      Eout  = Ein.*cos((pi/2).*(U1t - U0 + 4.1)/U_pi);
  end
else
  U1f   = fft(U1t);
  U1t   = ifft(U1f.*H);
  ...
  U2t   = ifft(U2f.*H);
  if C ~= 0
      Eout = Ein.*cos((pi/2).*(U1t - U2t - U0 + 4.1)/U_pi).*exp(-1j*(pi/2).*((U1t + U2t)/U_pi));
  else
      Eout = Ein.*cos((pi/2).*(U1t - U2t - U0 + 4.1)/U_pi);
  end
end
