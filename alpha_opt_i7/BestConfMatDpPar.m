...
for kk = melhorespontos
    ...
    BesResMat(9,MatPos) = To_Eval(kk).Fid;
    BesResMat(10,MatPos) = length(To_Eval(kk).Pea);
    BesResMat(11,MatPos) = BestPlanicity(MatPos);
    MatPos = MatPos + 1;
end
MatPos = MatPos - 1;
Vet_Lab = [{'Frequência Central [Hz]'}...
           {'Fase entre Eletrodos'}...
           {'Tensão Braço 1 [V]'}...
           {'Tensão Braço 2 [V]'}...
           {'Tensão de Polarização VB1[V]'}... 
           {'Tensão de Polarização VB2[V]'}...
           {'Tensão de Polarização Fase[V]'}...
           {'Index dos Arquivos'}...
           {'Ganho'}...
           {'Picos Encontrados'}];
...
xlswrite(Name,[{'FC'};{'Pha'};{'VA1'};{'VA2'};{'V1b'};{'V2b'};{'Pha'};...
                           {'Fid'};{'NPe'};{'Gai'};{'Pla'}],sheet,'A1:A11');%Writing the data in the Excel file
...
%%
            ...
            L          = 10;
            U10        = BesResMat(5,round(str2double(PlotIt)));
            U20        = BesResMat(6,round(str2double(PlotIt)));
            U_pi1      = 3.1;
            U_pi2      = 3.25;
            eta1       = 89;% 89;
            eta2       =-12.7;%-12.7;
            ...
            nel        =  2.60;
            alfa_ins   =  5.1;
            phi_0      =  BesResMat(7,round(str2double(PlotIt)));
            alfa0      =  0.55;
            ...
            Eout1  = (CW./2).*cos((pi/2).*(U1t - U10  - V1pi0)/U1_pi).*exp1;
            Eout2  = (CW./2).*cos(((pi/2).*(U2t - U20 - V2pi0)/U2_pi)).*exp2;
            if phi_0~=0
