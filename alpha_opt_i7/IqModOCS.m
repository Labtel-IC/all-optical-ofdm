function [Eout] = IqModOCS (Ein,Isig,Qsig,U_pi2,Vbias,t)
...
U0    = Vbias;
U1t   = Qsig;
U2t   = exp(-1j*pi/2).*Qsig;
U_pi  = U_pi2;
Vphi  = (U1t - U2t - U0);
EoutQ = Ein.*(1/2).*cos(Vphi.*(pi/(2*U_pi))-pi/2).*exp(-1j*(pi/2).*((U1t + U2t)/U_pi));
