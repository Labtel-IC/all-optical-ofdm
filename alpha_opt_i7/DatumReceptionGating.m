    ...
    %         TxSigToPlot = dpskmod(TxDataMat(ThisCarr,2:end),M);
            ...
            switch OfdMod
                case 'qam'
                    ...
                    if exist('ChanelEqualizer2','var')
                        equa = ChanelEqualizer2(ObsCarrUsed(ThisCarr),:);
                        equ = reshape(equa,length(equa)/NumFra,NumFra);
                        SigRecep3 = ((SigRecep3)./equ);%Fixing phase rotation and amplitude variation with the equalizer
                    end
                    if exist('ChanelEqualizer3','var')
                        equa = ChanelEqualizer3(ObsCarrUsed(ThisCarr),:);
                        equ = reshape(equa,length(equa)/NumFra,NumFra);
                        SigRecep3 = ((SigRecep3)./equ);%Fixing phase rotation and amplitude variation with the equalizer
                    end
...
%                     SigRecep4 = SigRecep4(:);                              %Serializing the signal
            ...
            EvmMatRec(ObsCarrPos==ThisCarr,:) = SigRecep3(:);                       %Taking just the middle samples as references
            [EvmDB(CurrentTest,ThisCarr), EvmPer(CurrentTest,ThisCarr), EvmRms(CurrentTest,ThisCarr) ] = EvmCalc( EvmMatRef(ObsCarrPos==ThisCarr,:),SigRecep3(:).' );
            [EvmDBJ(CurrentTest,ThisCarr),EvmPerJ(CurrentTest,ThisCarr),EvmRmsJ(CurrentTest,ThisCarr)] = evm1(M,OfdMod,EvmMatRef(ObsCarrPos==ThisCarr,:),SigRecep3(:).');
    ...
    case 'DPSK'
        ...
        for ThisCarr=InitCarr:CarrPass:NumCarr                                    %For each carrier the same process of reception will be used.
            if CarrUsedDo(ThisCarr)
                ...
                PassGate = round(Nb*NPPB/round(Nb*T*ThisCarr*fc*4));
                if PassGate<2
                    PassGate = 1;
                else
                    if mod(PassGate,2)
                ...
                RecSig1   = ESync1(VectGate);
                RecSig2   = ESync2(VectGate);
%                 RecSig   = RecSig./max(abs(RecSig));
                RecF     = f(VectGate);
                RecT     = t(VectGate);
                figure; hold all;
                plot(t,ESync1);
                plot(t(VectGate),RecSig1);
                plot(t(VectGate),RecSig2);
                 set(gcf,'units','normalized','outerposition',[0 0 1 1]);
                figure; hold all;
                plot(f,20*log10(abs(fftshift(fft(ESync1)./length(ESync1)))));
                plot(f,20*log10(abs(fftshift(fft(ESync2)./length(ESync2)))));
                plot(RecF,20*log10(abs(fftshift(fft(RecSig1)./length(RecSig1)))));
                plot(RecF,20*log10(abs(fftshift(fft(RecSig2)./length(RecSig2)))));
                 set(gcf,'units','normalized','outerposition',[0 0 1 1]);
                ...
                ESync1 = RecSig1.*conj(RecSig1);
                %             ESync1 = ifft(fft(ESync1)./fft(PulseResp(VetThisCarr==ThisCarr,:)));
                ESync2 = RecSig2.*conj(RecSig2);
                %             ESync2 = ifft(fft(ESync2)./fft(PulseResp(VetThisCarr==ThisCarr,:)));

                %%           Creating the Reception Filter
                
                
                [BitFilt,~] = FiltroGaussiano(f(VectGate),BWD2,CenFeq2,FiltOrd2);        %Creating filter for selection of the received signal
                BitFilt = fftshift(BitFilt);                                   %Shifting the filter for matching the received signal
...
%                 plot(RecF,20*log10(abs(fftshift(fft(Esync)./length(Esync)))));
                SyncAux   = Esync(5*NPPBG:19*NPPBG);                         %Selecting just the symbol to synchronize
                SyncSymb  = [zeros(1,10) ones(1,4) zeros(1,10)];
                SyncSymb  = rectpulse(SyncSymb,NPPBG);
                SyncedAux = SyncSymb(5*NPPBG:19*NPPBG);                      %Selecting just the symbol to synchronize

                %%                   Plot for Qualitative analizes
                            PrintInfo(PlotingThis*15,RecT(5*NPPBG:19*NPPBG),...
                            Esync(5*NPPBG:19*NPPBG),SyncSymb(5*NPPBG:19*NPPBG),ESync1(5*NPPBG:19*NPPBG),ESync2(5*NPPBG:19*NPPBG));
...
%                 RecSig   = RecSig./max(abs(RecSig));
                ...
                clear IxAux;
                if NPPBG==1
                    PosAuxEout = NPPBG:length(RecSig);                   %Varriable respossible to take just the samples at the middle of the symbol
                else
                    PosAuxEout = MidPoint:NPPBG:length(RecSig);                   %Varriable respossible to take just the samples at the middle of the symbol
                end
                
                IxAux      = RecSig(PosAuxEout);                          %Normalizing the reference
                a=0;
                EvmMatRec(ObsCarrPos==ThisCarr,1:length(IxAux)) = IxAux;                       %Taking just the middle samples as references
                RxSymbAmos(CurrentTest,ObsCarrPos==ThisCarr,1:length(IxAux)) = IxAux;%RxSymbAmosUp = [];
                [EvmDB(ObsCarrPos==ThisCarr), EvmPerA(ObsCarrPos==ThisCarr), EvmRmsA(ObsCarrPos==ThisCarr) ] = EvmCalc( EvmMatRef(ObsCarrPos==ThisCarr,1:length(IxAux)),IxAux );
                [EvmDBJ(ObsCarrPos==ThisCarr),EvmPerJA(ObsCarrPos==ThisCarr),EvmRmsJA(ObsCarrPos==ThisCarr)] = evm1(2,'dpsk',EvmMatRef(ObsCarrPos==ThisCarr,1:length(IxAux)),IxAux);

%##########################################################################
%                 Esync = Esync(1+SyncPeriod:end-SyncPeriod);
                if NPPBG==1
                    PosIx = NPPBG:length(RecSig);                                %Possition of the central samples - but the number of samples per symbol is even ????
                else
                    PosIx = MidPoint:NPPBG:length(RecSig);                                %Possition of the central samples - but the number of samples per symbol is even ????
                end
                IxAux = RecSig(PosIx);                                             %From the main received signal just a few samples are taken for further evaluation
                ...
                %eye diagram.
                IxAuxAB = RecSig(PosIx);       %Taking just those values relative to the uper eye
                InterAB = linspace(Perce*(min(RecSig)),Perce*(max(RecSig)),n);                     %Building the histogram boxes
...
%##########################################################################
                if CalcS
                    [~,~,EyeOpenI,EyeOpenHighI,EyeOpenLowI] = Olho(RecSig,T,NPPBG,0);
...
%                 [~,~,EyeOpenQ,EyeOpenHighQ,EyeOpenLowQ] = Olho(EoutQ,T,NPPB,0);
                ...
                TimGat = t(VectGate);
                if PrintinEye
                    PrintInfo(PlotingThis*17,RecSig,T,NPPBG);
                    hold on;
                    plot(TimGat(MidPoint),InterAB(round(SeqFinAB(LocMaxAB)-MaxValAB)),'k^');
                    plot(TimGat(MidPoint),InterAB(SeqFinAB(LocMaxAB)),'kv');
                    plot(TimGat(MidPoint),InterAB(round(SeqFinAB(LocMaxAB)-MaxValAB/2)),'k*');
                    if CalcS
                        plot(TimGat(MidPoint),EyeOpenLowI,'mx');
                        plot(TimGat(MidPoint),EyeOpenHighI,'mo');
                        plot(TimGat(MidPoint),EyeOpenLowI + EyeOpenI/2,'md');
                    end
                    a=0;
                end
...
%                     ))));
                ...
                for kk=1:NPPBG:length(RecSig)                                    %The comparison process will be made for each symbol period
                    %                 MeanOfData = mean(EoutI((kk-1)+SymLocI(1)));
                    if NPPBG==1
                        MeanOfData = mean(RecSig((kk-1)));
                    else
                        MeanOfData = mean(RecSig((kk-1)+MidPoint));
                    end
                ...
                BitErr(2)  = sum(xor(TxData,DataU));                       %Comparison between the Transmited and received and counting the differences
                BerDPSK(CurrentTest,ThisCarr) = BitErr(1)/(NbDPSK-(2*SyncPeriod));%Calculating the ration of wrong bits and the total number of bits transmited
                if BitErr(2)<BitErr(1)
                    BerDPSK(CurrentTest,ThisCarr) = BitErr(2)/(NbDPSK-(2*SyncPeriod));%Calculating the ration of wrong bits and the total number of bits transmited
                end
                %% Ploting the result for qualitative analizes
...
%                 PrintInfo(PlotingThis*22,t(IniSyncPos:SyncPos),EoutI(IniSyncPos:SyncPos),SyncSymb(IniSyncPos:SyncPos),EoutA(IniSyncPos:SyncPos),EoutB(IniSyncPos:SyncPos));
