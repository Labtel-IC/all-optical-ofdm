function [Eout,Count,MapVet]=OpticalIFFTGT(f,SymPer,StopPoint,E1,AngVec,...
                                                   ActStag,Count,E0,MapVet)
%%              All Optical Fast Furier Transform for N carriers
%c function [Eout,Count,MapVet]=OpticalFFTN(f,SymPer,StopPoint,E1,...
...
%c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%cEout,Count,MapVet]=OpticalFFTN(f,SymPer,StopPoint,E1,AngVec,...
                                                  % ActStag,Count,E0,MapVet)
%c    OpticalFFT
%c
%c  Input:
...
% to think in all possible steps .
    ...
    elseif nargin == 4%The minimum input arguments required.
        AngVec  = gpuArray(0);                                                       %The first angle is zero
        ActStag = gpuArray(1);                                                       %For the FFT at it's first order
        Count   = ones(1,StopPoint,'gpuArray');                                       %Control variable initialized
        E0      = zeros(size(E1,1),size(E1,2),size(E1,3),'gpuArray');                                     %Variable for the second input of the MZI
        MapVet  = gpuArray(1);                                                       %At the stage 1 the first carrier is 1
    elseif nargin<8
        E0     = zeros(size(E1,1),size(E1,2),size(E1,3),'gpuArray');                                      %Variable for the second input of the MZI
        MapVet = gpuArray(1);                                                        %At the stage 1 the first carrier is 1
    elseif nargin<9
    ...
    TimDel        = SymPer/(G2^ActStag);                                    %Create the time delay
    PhaDel        = AngVec(Count(ActStag));                                %Acquiring the time delay         
%     [Eout1,Eout2] = DelayInterfExp(t,TimDel,PhaDel,E1,E0);                 %Sends the input signal to the DI

    Count(ActStag) = Count(ActStag) + G1;                                   %Takin in account the current interaction
    ActStag        = ActStag + G1;                                          %Marking the occurence of the current stage
 ...
 %program It has got to an end.
    if ActStag>StopPoint
        MapVet = [MapVet MapVet+G2^(ActStag-G2)];
        [~,Eout2] = DelayInterfG(f,TimDel,PhaDel,E1(:,:,MapVet(2)),E1(:,:,MapVet(1)));
        Eout   = Eout2;
    else%Otherwise move to the next stage
        n      = ActStag-G1;                                                %Auxiliar variable to calculate the step
        MyStep = Gpi/(G2^n);                                                 %Step for the phase delay
        AngVec = [AngVec AngVec+MyStep];                                   %Calculating the phase delay for this current interaction
        Vetaux = MapVet+G2^(ActStag-G2);                                     %Calculating the carrier of thie current interaction
        
        [EoutAux1,Count,MyVetAux1] = OpticalIFFTGT(f,SymPer,StopPoint,...
                                        E1,AngVec,ActStag,Count,E0,Vetaux);%Calculating the next interaction of the FFT
        [EoutAux2,Count,MyVetAux2] = OpticalIFFTGT(f,SymPer,StopPoint,...
                                        E1,AngVec,ActStag,Count,E0,MapVet);%Calculating the next interaction of the FFT
        [~,Eout2]              = DelayInterfG(f,TimDel,PhaDel,EoutAux1,EoutAux2);%Sends the input signal to the DI
        
        MapVet = [MyVetAux2 MyVetAux1];                                    %Storing the received mapping vector.
        Eout   = Eout2;                                                    %Storing the received Output from the FFT.
    end
end
