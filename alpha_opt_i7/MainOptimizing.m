...
for VetSnr=VetSnrIni:VetSnrPass:VerSnrEnd
    ...
    if exist('Ns','var')
        if UseOfdmElect ==1
            RxSigOfdmNoEq = zeros(NumCarr,Ns*NumFra);
            RxSigOfdm = zeros(NumCarr,Ns*NumFra);
        else
    ...
    for CurrentTest=1:ThisTest
        ...
        if (UseOfdmElect)&&(UsedModula==5)&&(NumCarr==2)
            ...
            TxData = zeros(2,NumFra*length(DmtMve));
            TxSymbAux = zeros(NumFra,length(DmtMve));
            TxDataMat = zeros(1,NumFra*length(DmtMve));
            TxSig = 0;
            ...
            TxSymbModA  = rectpulse(TxSymbModA,OvSam);
...
%                         [EoutAux1Gpu,~,VetThisCarrGpu]=OpticalFFTN(fgpu,T,MaxNumStagTx,EoutGpu);
...
%                                     ff = time2freq(tt);
                            ...
                            %to zero.
                            [EoutModAux,~]=MZM(freqGHz,EoutTx(VetThisCarrTx==(RefCarr-1+kk),:),U,L,U0,U_pi1,U_pi2,nopt,nel,C,alfa0);%Modulating individual carriers
...
%                                 [EoutAux1Gpu,~,~]=OpticalIFFTN(fgpu,T,MaxNumStagT,EoutModTemGpu);
                    ...
                    case 'DPSK'
                        ...
                        for kk=InitCarrDo:CarrPass:NumCarr                             %Generating different data for each carrier
                            if CarrUsedDo(kk)
                                ...
                                if AddCP==1
                                    TxAuxB = reshape(SigTx,NPPB,NbDPSK);
                                    if SetCpSampZer==1
                    ...
                    case '4PAM'
                        ...
                        for kk=InitCarrDo:CarrPass:NumCarr                             %Generating different data for each carrier
                            if CarrUsedDo(kk)                                          % Frist it is chosen to transmite a random information...
                                ...
                                if Which4PAM==1
                                    if ModSchem
                                        [EoutModAux] = MZM(freqGHz,EoutTx(VetThisCarrTx==(RefCarr-1+kk),:),U,L,U0,U_pi1,U_pi2,nopt,nel,C,alfa0);
                                    else
                                        [EoutModAux] = MZM(freqGHz,EoutTx(VetThisCarrTx==(RefCarr-1+kk),:),U,L,U0,U_pi1,U_pi2,nopt,nel,C,alfa0);
                                    end
                                else
...
%                     [EoutAux1Gpu,~,VetThisCarrGpu]=OpticalFFTN(fgpu,T,MaxNumStag,EoutGpu);
                        ...
                        for ThisCarr=InitCarrDo:CarrPass:NumCarr                                    %For each carrier the same process of reception will be used.
                            if CarrUsedDo(ThisCarr)
                                ...
                                for kk=1:Nb-NumBitDesc%length(Esync(ThisDataSize))                                    %The comparison process will be made for each symbol period
                                    ...
                                    MeanOfData = mean(Esync((kk-1)*NPPB+(NPPB/2)+(+NPPB/16)));
                        ...
                        for ThisCarr=InitCarrDo:CarrPass:NumCarr                                           %For each carrier the same process of reception will be used.
                            if CarrUsedDo(ThisCarr)
                                ...
                                for kk=1:Nb-NumBitDesc%length(EoutI(ThisDataSize))                                    %The comparison process will be made for each symbol period
                                    ...
                                    MeanOfData = mean(EoutI((kk-1)*NPPB+3*NPPB/4));
                                    if MeanOfData > LevelDec1%EyeOpenLowI+EyeOpenI/2                     %If it is the uper level the incoming data
                                        DataOddM(kk) = 1;                                 %is 1
                                    else                                                       %If it is the lowest level the incoming data
                                        DataOddM(kk) = 0;                                 %is 0
                                    end
                                    MeanOfData = mean(EoutI((kk-1)*NPPB+NPPB/4));
                                    if MeanOfData > LevelDec2%EyeOpenLowI+EyeOpenI/2                     %If it is the uper level the incoming data
                                        DataOddL(kk) = 1;                                 %is 1
                                    else                                                       %If it is the lowest level the incoming data
                                        DataOddL(kk) = 0;                                 %is 0
                    ...
                    case '4PAM'
                        ...
                        for ThisCarr=InitCarrDo:CarrPass:NumCarr                                           %For each carrier the same process of reception will be used.
                            if CarrUsedDo(ThisCarr)
                                ...
                                ContBit1  = 1;
                                ContBit2  = 1;
                                for kk=1:Nb-NumBitDesc%NPPB:length(Ix)                                       %The comparison process will be made for each symbol period
                        ...
                        for kk=InitCarrUp:CarrPass:NumCarr%Generating different data for each carrier
                            if CarrUsedUp(kk)
                                ...
                                if AddCP==1
                                    ...
                                    SigTx = [TxAux2 TxAux2(end-(StuffSampels-1):end)];
                                end
                                ...
                                %to zero.
                                [EoutModAux,~]=MZM(freqGHz,EoutT,U,L,U0,U_pi1,U_pi2,nopt,nel,C,alfa0);    %Modulating individual carriers
                            ...
                            if CarrUsedUp(kk)
                                ...
                                if Which4PAM
                                    if ModSchem
                                        ...
                                        [EoutModAux] = MZM(freqGHz,EoutT,U,L,U0Up,U_pi1,U_pi2,nopt,nel,C,alfa0);
                                    else
                                        [EoutModAux] = MZM(freqGHz,EoutT,U,L,U0Up,U_pi1,U_pi2,nopt,nel,C,alfa0);
                                    end
                                else
